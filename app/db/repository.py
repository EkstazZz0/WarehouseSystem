...
async def create_order(session: SessionDep) -> Order:
    order = Order()
    session.add(order)
    await session.commit()
...
async def update_orders_status_by_delivery(items: list[ItemSupply], session: SessionDep) -> None:
    for item in items:
        db_item = await session.get(Item, item.item_id)
        order_ids = (await session.execute(select(OrderItem.order_id).where((OrderItem.item_id == item.item_id) & (OrderItem.status == OrderItemStatus.on_delivery)))).scalars().all()

        if not order_ids:
            continue

        orders = (await session.execute(select(Order).where(Order.order_id.in_(order_ids)).order_by(Order.created_at))).all()

        for order in orders:

            receivable_quantity = (await session.execute(select(func.sum(OrderItem.quantity))
                                            .where((OrderItem.item_id == item.item_id) & (OrderItem.status == OrderItemStatus.receivable))
                                            .group_by(OrderItem.item_id))).scalars().first()
            if not receivable_quantity:
                receivable_quantity = 0
                
            order_order_item = (await session.execute(select(OrderItem)
                                            .where((OrderItem.order_id == order.order_id) & (OrderItem.item_id == item.item_id) & (OrderItem.status == OrderItemStatus.on_delivery)))).scalars().first()
            ...
            if order_order_item.quantity + receivable_quantity <= db_item.quantity:
                ...
                await session.commit()
                await session.refresh(order_order_item)
        
        order_ids = (await session.execute(select(OrderItem.order_id).where(OrderItem.item_id == item.item_id))).scalars().all()

        orders = (await session.execute(select(Order).where(Order.order_id.in_(order_ids)))).scalars().all()
        for order in orders:
            order_items = (await session.execute(select(OrderItem).where((OrderItem.order_id == order.order_id) & or_(OrderItem.status == OrderItemStatus.on_delivery, OrderItem.status == OrderItemStatus.receivable)))).scalars().all()
...
async def make_order(order_items: list[CreateOrder], session: SessionDep) -> OrderPublic:
    db_items = []
    
    for order_item in order_items:
        db_item = await session.get(Item, order_item.item_id)
...
async def generate_order_number(order_id: UUID, session: SessionDep) -> int:
    ...
    if max_order_queue_number is None:
        ...
        order_queue_number = get_next_order_number(order_numbers=session.execute(select(Order.queue_number).where(Order.queue_number != None).order_by(Order.queue_number)).all())
    else:
